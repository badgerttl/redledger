...
Reporting turns the assessment into decisions the client can act on. The report should be understandable to executives, reproducible for engineers, and specific enough that remediation teams know exactly what to fix.

## Objectives

- Communicate findings clearly to technical and non-technical audiences.
- Provide actionable, specific remediation guidance.
- Make every finding reproducible from the report alone.
- Preserve evidence quality, scope accuracy, and methodology transparency.
- Help the client prioritize remediation based on risk and business impact.

## Operator Flow

| Step | Goal | Output |
|------|------|--------|
| 1 | Verify scope and methodology | Accurate scope, exclusions, limits, and test window |
| 2 | Triage findings | Deduplicated, confirmed, severity-ordered findings |
| 3 | Write technical detail | Description, reproduction, affected assets, evidence |
| 4 | Write impact and remediation | Business impact and actionable fixes |
| 5 | Build executive narrative | Summary, themes, top risks, posture |
| 6 | Attach appendices | Raw outputs, screenshots, credentials redaction, limitations |
| 7 | Review and export | Final Markdown, HTML, or PDF report |

## Report Structure

Use this structure to keep the report predictable and easy to review.

### Executive Summary

- High-level overview for management and stakeholders; avoid technical jargon.
...
Severity should reflect exploitability, technical impact, business impact, and environmental context.
...
Good findings are specific, reproducible, and tied to impact. Avoid vague titles, generic remediation, or evidence that cannot be repeated.
...
Explain what is vulnerable, where it exists, and why it matters technically.
...
Be specific. Do not stop at "sanitize inputs."
...
Use this as a starting point, then tailor the language to the client and engagement type.
...
Consistent evidence naming makes report review and future retesting much easier.
...
Use priority to help teams sequence fixes when severity alone is not enough.
...
Use these as naming patterns so reports stay consistent across engagements.
...
## Checklist Workflow Reference

These headings intentionally match the Methodology checklist. Reporting items are less command-heavy than technical testing, so each section gives the concrete report workflow, checks, or template to complete that item.
...
Compare this against Scope entries, Activity Log entries, imported tool output, and the final report scope section.
...
Group related symptoms under one root-cause finding when remediation is the same.

### Complete all finding entries
...
Every confirmed finding should be reproducible by an engineer who did not participate in testing.
...
Use the finding evidence gallery and Tool Output links so report exports preserve context.

### Write reproduction steps

```text
1. Authenticate as [role] or access [unauthenticated endpoint].
2. Send the following request:
3. Observe [response / delay / file / shell / data].
4. Repeat with [control request] to confirm behavior.
5. Impact is demonstrated by [minimal proof].
```

Include exact URLs, parameters, headers, payloads, and expected results.
...
Score from exploitability and impact, then adjust narrative severity for business context. Use the CVSS quick reference above for common examples.

### Write business impact

```text
An attacker can [action] against [asset/data/process], resulting in [business consequence].
This could affect [customers/users/operations/compliance] because [reason].
```

Make the consequence concrete: data exposure, privilege gain, transaction abuse, operational outage, or compliance impact.
...
Recommended fix:
1. [Primary fix]
2. [Configuration/code change]
3. [Validation step]
4. [Compensating control if needed]
```

Prefer specific product settings, code patterns, version numbers, and verification commands over generic hardening advice.

### Map references and classifications
...
Use references to support remediation, not to replace the finding explanation.

### Write executive summary

```text
The assessment identified [N] findings: [N] Critical, [N] High, [N] Medium, [N] Low, and [N] Informational.
...
Immediate remediation should focus on [priority].
```

Keep this section business-focused and avoid tool names unless they matter to leadership.
...
This protects the report from implying coverage that was not actually performed.

### Proofread report

```text
Review checklist:
- Finding titles are specific.
...
Read the executive summary separately from the technical findings to make sure both audiences are served.
...
3. Review Markdown/HTML/PDF output.
4. Confirm evidence renders correctly.
5. Save the delivery version.
```

For Nmap and Burp imports, verify generated findings and assets before exporting.
...
Retest plans should tell the client exactly how closure will be verified.
...
Prepare both business talking points and technical appendix material for engineering follow-up.
